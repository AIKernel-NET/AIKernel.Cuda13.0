...
This is part of the AIKernel 0.1.3 publication preparation line. The package
validates external GPU Capability distribution without moving CUDA, LibTorch,
or native runtime payloads into AIKernel.Core.
...
1. Publish AIKernel.NET and AIKernel.Core managed dependencies first.
   ...
   may be a cache-busting build such as `0.1.3-dev{build-number}`; for the
   public v0.1.3 CUDA release, it should point at the already-published
   AIKernel.NET/Core package family selected for the release train.
